...
var BaseCollection = (function () {
  ...
  _createClass(BaseCollection, [{
    ...
    key: '_get',
    value: function _get(path, update, opts, cb) {
      ...
      self._api.get('collection/' + self.name + '/' + path, opts, function (err, res) {
        if (err) callback(err);else {
          if (update) {
            (0, _extend2['default'])(self, res.body);
            delete self.code;
            delete self.error;
          }
    ...
    key: '_put',
    value: function _put(path, data, update, cb) {
      ...
      self._api.put('collection/' + self.name + '/' + path, data, function (err, res) {
        if (err) callback(err);else {
          if (update) (0, _extend2['default'])(self, res.body);
          callback(null, res.body);
        }
      });
      return promise;
...
var DocumentCollection = (function (_BaseCollection2) {
  function DocumentCollection() {
    ...
    if (_BaseCollection2 != null) {
      _BaseCollection2.apply(this, arguments);
    }
  }
...
var EdgeCollection = (function (_BaseCollection3) {
  function EdgeCollection() {
    ...
    if (_BaseCollection3 != null) {
      _BaseCollection3.apply(this, arguments);
    }
  }

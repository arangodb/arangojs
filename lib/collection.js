...
extend(Collection.prototype, {
  ...
  document: function (documentHandle, callback) {
    ...
    self._connection.get('document/' + documentHandle, function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
  ...
  save: function (data, opts, callback) {
    ...
    if (!callback) callback = noop;
    opts = extend({}, opts, {collection: this.name});
    self._connection.post('document', data, opts, function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
      else callback(null, body);
    });
  ...
  replace: function (documentHandle, data, opts, callback) {
    ...
    if (!callback) callback = noop;
    opts = extend({}, opts, {collection: this.name});
    self._connection.put('document/' + documentHandle, data, opts, function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
      else callback(null, body);
    });
  ...
  update: function (documentHandle, data, opts, callback) {
    ...
    if (!callback) callback = noop;
    opts = extend({}, opts, {collection: this.name});
    self._connection.patch('document/' + documentHandle, data, opts, function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
      else callback(null, body);
    });
  ...
  remove: function (documentHandle, opts, callback) {
    ...
    if (!callback) callback = noop;
    opts = extend({}, opts, {collection: this.name});
    self._connection.delete('document/' + documentHandle, opts, function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
      else callback(null);
    });
  ...
  all: function (opts, callback) {
    if (!callback) callback = noop;
    opts = extend({}, opts, {collection: this.name});
    self._connection.get('document', function (err, body) {
      if (err) callback(err);
      else if (body.error) callback(new ArangoError(body));
      else callback(null, body);
    });

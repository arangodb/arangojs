...
extend(BaseCollection.prototype, {
  ...
  replace: function (documentHandle, data, opts, callback) {
    ...
    this._connection.put((this.type === 3 ? 'edge/' : 'document/') + documentHandle, data, opts, function (err, body) {
      if (err) callback(err);
  ...
  update: function (documentHandle, data, opts, callback) {
    ...
    this._connection.patch((this.type === 3 ? 'edge/' : 'document/') + documentHandle, data, opts, function (err, body) {
      if (err) callback(err);
      else callback(null, body);
    });
  },
  remove: function (documentHandle, opts, callback) {
    ...
    this._connection.delete((this.type === 3 ? 'edge/' : 'document/') + documentHandle, opts, function (err, body) {
      if (err) callback(err);
      else callback(null);
    });
  },

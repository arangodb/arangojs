...
extend(Database.prototype, {
  ...
  getBatch:function() {
      var batch = new Database(this._connection.config);
      // To avoid socket creation multiplication to the same host
      batch._connection._request = this._connection._request;
      batch.run = function(callback) {
          batch._connection.runBatch(callback);
      };
      batch.start = function() {
          batch._connection.startBatch();
      };
      return batch;
  }
});
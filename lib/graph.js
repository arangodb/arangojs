...
extend(Graph.prototype, {
  _post: function (path, data, callback) {
    ...
    this._api.post('gharial/' + this.name + '/' + path, data, callback);
  },
  _delete: function (path, data, callback) {
    if (!callback) callback = noop;
    this._api.delete('gharial/' + this.name + '/' + path, data, callback);
  },
  drop: function (dropCollections, callback) {
    ...
    if (!callback) callback = noop;
    this._api.delete('gharial/' + this.name, {
...
extend(VertexCollection.prototype, {
  ...
  vertex: function (documentHandle, callback) {
    if (!callback) callback = noop;
    this._api.get(this._documentPath(documentHandle), function (err, body) {
      if (err) callback(err);
  ...
  save: function (data, callback) {
    if (!callback) callback = noop;
    this._api.post(this._relativePath(''), data, {

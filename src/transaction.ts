...
export class Transaction {
   ...
   * await trx.run(() => col.save({ hello: "world" }));
   ...
   * const left = await trx.run(() => vertices.save({ label: "left" }));
   * const right = await trx.run(() => vertices.save({ label: "right" }));
   *
   * // Results from preceding actions can be used normally
   * await trx.run(() => edges.save({
   *   _from: left._id,
   *   _to: right._id,
   *   data: "potato"
   * }));
   ...
   * // transaction in a separate trx.run call
   * await trx.run(() => collection.save(data));
   * await trx.run(() => collection.save(moreData));
   * ```
   *
   * @example
   * ```js
   ...
   * });
   *
   * // BETTER: Perform any async logic needed outside the trx.run call
   * await doSomethingElse();
   * await trx.run(() => collection.save(data));
   *
   ...
   * }
   * await trx.run(() => saveSomeData()); // WRONG
   *
   ...
   *   await trx.run(() => collection.save(moreData));
   * }
   * await saveSomeData(); // no trx.run call needed
   * ```
   *
   * @example
   * ```js
   * // BAD! You must for the promise to resolve (or await on the trx.run call)
   * // before calling trx.run again!
   * trx.run(() => collection.save(data)); // WRONG
   * await trx.run(() => collection.save(moreData));
   *
   * // BAD! The trx.run callback can not make multiple calls to async arangojs
   * // methods, not even using Promise.all!
   * await trx.run(() => Promise.all([ // WRONG
   *   collection.save(data),
   *   collection.save(moreData),
   * ]));
   *
   * // BAD! Multiple trx.run calls can not run in parallel!
   * await Promise.all([ // WRONG
   *   trx.run(() => collection.save(data)),
   *   trx.run(() => collection.save(moreData)),
   * ]));
   *
   * // BETTER: Always call trx.run sequentially, one after the other
   * await trx.run(() => collection.save(data));
   * await trx.run(() => collection.save(moreData));
   *
   * // OKAY: The then callback can be used if async/await is not available
   * trx.run(() => collection.save(data))
   *   .then(() => trx.run(() => collection.save(moreData)));
   * ```
   *
   * @example
   * ```js
   ...
   * const collection = await trx.run(() => db.collection("my-documents"));
   *
   * // BETTER: If an arangojs method is not async and doesn't return promises,
   * // call it without trx.run
